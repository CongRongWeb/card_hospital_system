    ...
    <div>
      <el-row :gutter="0">
        <el-form ref="elForm2" :model="formData2"  label-width="85px" size="small">
          ...
          <el-col :span="24">
            <el-form-item size="large">
              <el-button type="primary" size="small" plain @click="addUser" v-if="this.$store.uutype=='1'">添加</el-button>
            </el-form-item>
          </el-col>
        </el-form>
      </el-row>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              size="mini"
              v-if="scope.row[7]==0"
              @click="quyaoFn( scope.row)">取药</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import {getMedicine, getPatient,getChufang,insertChufangOrUpdate,getChufangDetail,quyaoApi} from "../../api/index"
export default {
  name: "index",
  data() {
    return {
  ...
  methods: {
    ...
    getChufangDetailFn(data){
      ...
      getChufangDetail({name:data[1]}).then(res=>{
        let  arr = res.data[0][4].split(',')
        let  numArr =res.data[0][5].split(',')
