<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-row style="padding: 10px 0 40px 0">
          <el-radio v-model="ruleForm.uutype" label="0">管理员</el-radio>
          <el-radio v-model="ruleForm.uutype"  label="1">医生</el-radio>
<!--          <el-radio v-model="ruleForm.uutype"  label="2">护士</el-radio>-->
        <el-radio v-model="ruleForm.uutype"  label="3">药房</el-radio>
      </el-row>
        ...
        if (valid) {
          ...
          login(this.ruleForm).then(res=>{
            if(res.success===1){
              ...
              // this.lastCity=sessionStorage.getItem('address')
              // console.log(this.lastIp)
              // console.log(this.lastCity)
/*              const DATA={
                uuid:res.uuid,
                ipAddress:sessionStorage.getItem('ip'),
                cityAddress:sessionStorage.getItem('address')
              }
              writeloginLog(DATA).then(rres=>{
                if(rres.success===1){
                  this.$store.uutype=this.ruleForm.uutype

                  this.$store.uuid=res.uuid
