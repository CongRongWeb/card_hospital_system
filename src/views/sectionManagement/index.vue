    ...
    <el-dialog
      ...
      <el-form ref="elForm" :model="formData" :rules="rules" label-width="100px" size="medium">
        <el-form-item label="名称" prop="name">
          <el-input v-model="formData.name" :disabled="disabled" :style="{width: '100%'}" clearable placeholder="请输入科室名称" ></el-input>
        </el-form-item>
    ...
    <div>
      <el-row :gutter="0">
        <el-form ref="elForm2" :model="formData2"  label-width="85px" size="small">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="科室名称" prop="name">
                  <el-input v-model="formData2.name" :style="{width: '100%'}" clearable placeholder="请输入科室名称">
                  </el-input>
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <el-col :span="24">
            <el-form-item size="large">
              <el-button type="primary" size="small" plain @click="addUser">添加科室</el-button>
            </el-form-item>
          </el-col>
        </el-form>
      </el-row>
...
<script>
import {delSection, getSectionList, insertSectionOrUpdate} from "../../api/index"
export default {
  name: "index",
  data() {
    return {
  ...
  methods: {
    ...
    addFn() {
      this.$refs['elForm'].validate(valid => {
        if (!valid) return
        insertSectionOrUpdate(this.formData).then(res=>{
          if(res.flag){
            this.$nextTick(()=>{
    ...
    handleDelete(data){
      delSection({uuid:data[0]}).then(res=>{
        if(res.flag){
          this.$message.success(res.message)
          this.refreshView()
        }
